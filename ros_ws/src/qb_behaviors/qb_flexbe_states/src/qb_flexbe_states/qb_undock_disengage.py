...
class QbUndockDisengageState(EventState):
    '''
    ...
    def __init__(self, rotation_speed, rotation_angle):
        super(QbUndockDisengageState, self).__init__(outcomes=['failed', 'done'])
    ...
    def on_enter(self, userdata):
        Logger.loginfo("UNDOCK DISENGAGE STARTED!")
        self._start_time = rospy.Time.now()
        
    def on_exit(self, userdata):
        Logger.loginfo("UNDOCK DISENGAGE ENDED!")
        
    def on_start(self):
        Logger.loginfo("DOCK DISENGAGE READY!")
        
    def on_stop(self):
        Logger.loginfo("UNDOCK DISENGAGE STOPPED!")
